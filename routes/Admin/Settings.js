...
router.post('/admin/settings/change/theme/color', isAdmin, async (req, res) => {
  ...
  const themePath = path.join(__dirname, '..', '..', 'storage', 'theme.json');
...
router.post('/admin/settings/sendTestEmail', isAdmin, async (req, res) => {
  try {
    const { recipientEmail } = req.body;
    await sendTestEmail(recipientEmail);
    logAudit(req.user.userId, req.user.username, 'SMTP:testEmail', req.ip);
...
router.post('/admin/settings/change/logo', isAdmin, upload.single('logo'), async (req, res) => {
  ...
  try {
    ...
    if (type === 'image' && req.file) {
      settings.logo = true;
      await db.set('settings', settings);
      logAudit(req.user.userId, req.user.username, 'logo:uploaded', req.ip);
      res.redirect('/admin/settings');
    } else if (type === 'none') {
      ...
      settings.logo = false;
      await db.set('settings', settings);
      logAudit(req.user.userId, req.user.username, 'logo:removed', req.ip);
      res.redirect('/admin/settings');
    } else {
